...
function Get-OutlookDataFilesWorkgroup {
    ...
    try {
        # Access remote computer's C$ share
        $remotePath = "\\$ComputerName\C$\Users"
        ...
        if ($driveLetter) {
            try {
                $null = New-PSDrive -Name $driveLetter -PSProvider FileSystem -Root $remotePath -Credential $Credential -ErrorAction Stop
                
                $profiles = Get-ChildItem "$($driveLetter):" -Directory -ErrorAction SilentlyContinue | 
                    Where-Object { $_.Name -notin @('Public', 'Default', 'Default User', 'All Users') }
                
                Write-Log "Found $($profiles.Count) user profiles on $ComputerName" "INFO"
                
                foreach ($profile in $profiles) {
                    Write-Log "Scanning profile: $($profile.Name) on $ComputerName" "INFO"
                    
                    # Expanded Outlook data file locations - search more deeply
                    $searchPaths = @(
                        # Standard locations
                        "$($driveLetter):\$($profile.Name)\Documents\Outlook Files",
                        "$($driveLetter):\$($profile.Name)\AppData\Local\Microsoft\Outlook",
                        "$($driveLetter):\$($profile.Name)\AppData\Roaming\Microsoft\Outlook",
                        
                        # Additional common locations
                        "$($driveLetter):\$($profile.Name)\Documents",
                        "$($driveLetter):\$($profile.Name)\Desktop",
                        "$($driveLetter):\$($profile.Name)\Downloads",
                        "$($driveLetter):\$($profile.Name)\OneDrive\Documents",
                        "$($driveLetter):\$($profile.Name)\OneDrive - *\Documents"
                        "$($driveLetter):\$($profile.Name)\OneDrive\ドキュメント"
                    )
                    
                    Write-Log "Searching in standard locations..." "INFO"
                    
                    # First, check standard locations
                    foreach ($path in $searchPaths) {
                        if (Test-Path $path) {
                            Write-Log "  Checking: $path" "INFO"
                            $files = Get-ChildItem -Path $path -Include "*.pst", "*.ost" -File -ErrorAction SilentlyContinue
                            
                            if ($files) {
                                Write-Log "  Found $($files.Count) file(s) in $path" "SUCCESS"
                            }
                            
                            foreach ($file in $files) {
                                # Convert back to UNC path for backup
                                $uncPath = $file.FullName -replace "^$($driveLetter):", "\\$ComputerName\C$\Users"
                                
                                # Check if already added (avoid duplicates)
                                $alreadyExists = $dataPaths | Where-Object { $_.FilePath -eq $uncPath }
                                if (-not $alreadyExists) {
                                    Write-Log "  Added: $($file.Name) ($([math]::Round($file.Length/1MB, 2)) MB)" "SUCCESS"
                    ...
                    # Deep recursive search in Documents and Desktop (but limit depth)
                    Write-Log "Performing deep search in Documents and Desktop folders..." "INFO"
                    ...
                    foreach ($deepPath in $deepSearchPaths) {
                        if (Test-Path $deepPath) {
                            try {
                                Write-Log "  Deep searching: $deepPath" "INFO"
                                $files = Get-ChildItem -Path $deepPath -Include "*.pst", "*.ost" -Recurse -Depth 5 -File -ErrorAction SilentlyContinue
                                
                                if ($files) {
                                    Write-Log "  Found $(@($files).Count) file(s) in deep search of $deepPath" "SUCCESS"
                                }
                                ...
                                foreach ($file in $files) {
                                    ...
                                    $alreadyExists = $dataPaths | Where-Object { $_.FilePath -eq $uncPath }
                                    if (-not $alreadyExists) {
                                        Write-Log "  Added from deep search: $($file.Name) ($([math]::Round($file.Length/1MB, 2)) MB)" "SUCCESS"
                                        $dataPaths += [PSCustomObject]@{
                                            ComputerName = $ComputerName
                                            UserProfile = $profile.Name
                                            FilePath = $uncPath
                                ...
                                Write-Log "Error searching $deepPath : $_" "WARNING"
                            }
                        }
                    }
                }
                ...
                Write-Log "Error accessing $ComputerName : $_" "ERROR"
                Remove-PSDrive -Name $driveLetter -Force -ErrorAction SilentlyContinue
            }
        }
    }
    catch {
        Write-Log "Error scanning $ComputerName : $_" "ERROR"
    }
...
function Close-RemoteOutlook {
    ...
    try {
        ...
        if ($outlookProcesses) {
            ...
            $notificationMessage = "Shutup Monika.`n`u!"
